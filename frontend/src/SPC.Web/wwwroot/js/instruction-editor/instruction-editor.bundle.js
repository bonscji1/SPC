...
.tippy-box[data-animation=fade][data-state=hidden] {
  ...
  // src/SPC.Web/wwwroot/js/instruction-editor/mention-picker.js
  function escapeHtml(value) {
    return String(value).replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;");
  }
  function positionPicker(element, clientRect2) {
    if (!clientRect2) {
      return;
    }
  ...
  function renderPickerList(element, items, selectedIndex, onPick) {
    ...
    items.forEach((item, index) => {
      ...
      button.innerHTML = `<span>${escapeHtml(item.label)}</span><span class="editor-picker-meta">${escapeHtml(item.kind)} \xB7 ${escapeHtml(item.detail ?? "")}</span>`;
  ...
  function createTiptapMentionSuggestion(getItems) {
    return {
      char: "#",
      allowSpaces: false,
      command: ({ editor, range, props }) => {
      ...
      items: ({ query }) => {
        ...
        return getItems().filter((item) => item.name.toLowerCase().includes(normalized)).slice(0, 8).map((item) => ({
          id: String(item.id),
          label: item.name,
          kind: item.kind,
          detail: item.detail ?? ""
  ...
  // src/SPC.Web/wwwroot/js/instruction-editor/tiptap-step-editor.js
  var IngredientMention = Mention.extend({
    ...
    renderHTML({ node, HTMLAttributes }) {
      ...
      return [
        ...
        node.attrs.detail ? ["span", { class: "instruction-chip-tip", role: "tooltip" }, node.attrs.detail] : ""
      ];
    },
    renderText({ node }) {
  ...
  function bindToolbar(toolbar, editor) {
    ...
    toolbar.querySelectorAll("[data-cmd]").forEach((button) => {
      button.addEventListener("click", () => {
        const command2 = button.dataset.cmd;
        if (command2 === "bold") {
          editor.chain().focus().toggleBold().run();
        } else if (command2 === "italic") {
          editor.chain().focus().toggleItalic().run();
        } else if (command2 === "bulletList") {
  ...
  function updateMentions(id, mentionItems) {
  ...
  // src/SPC.Web/wwwroot/js/instruction-editor/instruction-editor.js
...
//# sourceMappingURL=instruction-editor.bundle.js.map
