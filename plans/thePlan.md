...
**Status:** in progress — steps 1–8 and 10–12 done; next product step is nutrition API (step 9).
...
| 12 | Sign up — self-serve extra login accounts | **done** | [step12-signup.md](./step12-signup.md) |
                            ...
                            └── step5-save-recipes-and-ingredients
                                    ...
                                    └── step10-backend           # discuss DB first; also uses step 6 compose slots
                                            └── step11-login-user   # Blazor login + Api* repos; no fake local auth
                                                    └── step12-signup       # self-serve accounts (extra users)
```

Step 9 (nutrition API) is the next **product** step and does not wait on accounts. Cookbook browsing lives on Home (step 7).
...
- When do we introduce a backend vs. localStorage-only persistence? **Step 5 was the localStorage prototype.** API + DB is [step 10](./step10-backend.md). Blazor login is [step 11](./step11-login-user.md). Self-serve sign-up is [step 12](./step12-signup.md) (**done**).
